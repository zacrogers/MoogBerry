...
def envelope():
    ...
    #GPIO.output(mux3s1, 1)
    #GPIO.output(mux3s2, 1)
    release = int(mcp2.read_adc(2))
    releaseOSC = OSC.OSCMessage("/env/r")
    releaseOSC.append(release/991.9)
    c.send(releaseOSC)

    if(DEBUG):
	print("A-",attack,"D-", decay, "S-", sustain, "R-", release)
...
def oscillator1():
    #reading from adc
    osc1Semi = int(mcp1.read_adc(5))
    osc1Octave = int(mcp1.read_adc(6))
    osc1Cents = int(mcp1.read_adc(7))
    osc1PWM = int(mcp2.read_adc(6))
...
def oscillator2():
    #reading from adc
    osc2Semi = int(mcp1.read_adc(0))
    osc2Octave = int(mcp1.read_adc(1))
    osc2Cents = int(mcp1.read_adc(2))
    osc2PWM = (mcp1.read_adc(3))
    osc2Vol = (mcp1.read_adc(4))
    if(DEBUG):
...
def master():
    ...
    GPIO.output(mux3s1 ,1)
    GPIO.output(mux3s2, 1)    
    subVol = int(mcp2.read_adc(0))
    GPIO.output(mux3s0, 0)
    GPIO.output(mux3s1, 0)
    GPIO.output(mux3s2, 1)
    glide = (mcp2.read_adc(0))
    bend = (mcp2.read_adc(0))
